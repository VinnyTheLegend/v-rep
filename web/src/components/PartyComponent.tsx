  ...
  return (
    ...
    <div className="code-container">
      <form action="" id="join-party-form">
        <input type="text" id="code-input" name="code-input" value={joinInput} onChange={(e) => setJoinInput(e.target.value)}></input>
        <button id="code-submit" className="btn" type="button" onClick={joinRequest}>
          Join
        </button>
      </form>
              ...
              <div className="filler"></div>
                {partyData.leader === member.citizenid ? (
                  ...
                  playerInfo?.citizenid === partyData.leader && (
                    <button className="btn btn-kick btn-danger" onClick={() => kickRequest(member.cid)}>
                      <FontAwesomeIcon
                        icon={faUserXmark}
                        style={{ color: "#6060b8" }}
                      />
